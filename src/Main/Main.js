...
async function getNYTimes(numDisplay) {
	const response = await fetch(
		'https://api.nytimes.com/svc/mostpopular/v2/emailed/1.json?api-key=8jX3EGImSIW8Js6xembfQKaVtBdMChE2'
	);
	const result = await response.json();

	let articles = [];
	for (let i = 0; i < result.results.length; i++) {
		if (articles.length < numDisplay && result.results[i].media.length !== 0) {
			...
			const thumbnail = result.results[i].media[0]['media-metadata'][0].url;
...
async function getGuardianCoding(numDisplay) {
	const response = await fetch(
		'https://content.guardianapis.com/search?show-fields=thumbnail&page-size=100&q=artificial%20intelligence%20computer%20programming%20software%20development&api-key=1b06a9cd-cdd9-415c-b922-0818623b9009'
	);
							...
							<section className='container'>
								...
								<nav>
									<ul>
										<li>
											<a href='#'>
												News Source: <span>{this.state.currentSource}</span>
											</a>
											<SourceSelector
									...
									<section id='search'>
										<input
											...
											name='name'
											value={
												this.state.shouldClear ? null : this.state.searchTerm
											}
											style={
							...
							<div>
								<ul>
									{currentSourceFeed.map((article) =>
										isMatch(article, this.state.searchTerm) ? (
											<li>
												<Article
													article={article}
													articleToPop={this.state.articleToPop}
													shouldPop={this.state.shouldPop}
